...
void jtest_dump_str_segments(void)
{
    ...
    uint32_t seg_cnt     = (strlen(JTEST_FW.str_buffer) / JTEST_STR_MAX_OUTPUT_SIZE) + 1;
    ...
    for (seg_idx = 0; seg_idx < seg_cnt; ++seg_idx) {
        ...
        if (seg_idx < JTEST_STR_MAX_OUTPUT_SEGMENTS) {
            ...
            while (memmove_idx < (seg_cnt - seg_idx - 1)) {
                memmove(JTEST_FW.str_buffer + (memmove_idx * JTEST_STR_MAX_OUTPUT_SIZE),
                        JTEST_FW.str_buffer + ((memmove_idx + 1) * JTEST_STR_MAX_OUTPUT_SIZE), JTEST_BUF_SIZE);
                ++memmove_idx;
            }
        }
    }
