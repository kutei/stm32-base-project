...
#define ARR_DESC_DEFINE(type, name, element_count, content) \
    ...
    ARR_DESC_INTERNAL_DEFINE(name, &ARR_DESC_ARR_NAME(name), element_count, sizeof(type)) /* Note the lacking semicolon */
...
#define ARR_DESC_DEFINE_SUBSET(name, original, element_cnt)                                                                                         \
    ARR_DESC_INTERNAL_DEFINE(name, &ARR_DESC_ARR_NAME(original), element_cnt, sizeof(ARR_DESC_ARR_NAME(original)[0])) /* Note the lacking semicolon \
                                                                                                                       */
...
#define ARR_DESC_DEFINE_USING_ARR(type, name, array_ptr, start_idx, element_cnt) \
    ARR_DESC_INTERNAL_DEFINE(name, (type *)(array_ptr + start_idx), element_cnt, sizeof(type)) /* Note the lacking semicolon*/
...
#define ARR_DESC_MEMCPY(arr_desc_dest_ptr, arr_desc_src_ptr, bytes)                                                                 \
    do {                                                                                                                            \
        memcpy((arr_desc_dest_ptr)->data_ptr, (arr_desc_src_ptr)->data_ptr, BOUND(0, (arr_desc_dest_ptr)->underlying_size, bytes)); \
    } while (0)
 ...
 */
#define ARR_DESC_COPYABLE(arr_desc_dest_ptr, arr_desc_src_ptr) (ARR_DESC_BYTES(arr_desc_src_ptr) <= (arr_desc_dest_ptr)->underlying_size)
