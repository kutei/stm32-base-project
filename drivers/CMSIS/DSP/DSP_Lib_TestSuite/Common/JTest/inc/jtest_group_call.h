...
#define JTEST_GROUP_DUMP_RESULTS(group_ptr)                                                                   \
    do {                                                                                                      \
        JTEST_DUMP_STRF("Tests Run: %" PRIu32                                                                 \
                        ...
                        "\n"                                                                                  \
                        "   Failed: %" PRIu32 "\n",                                                           \
                        (group_ptr)->passed + (group_ptr)->failed, (group_ptr)->passed, (group_ptr)->failed); \
    } while (0)
...
#define JTEST_GROUP_CALL(group_fn)                                                                   \
    do { /* Save the current group from JTEST_FW_t before swapping */                                \
        ...
        JTEST_GROUP_UPDATE_PARENT_GROUP_OR_FW_PF(JTEST_CURRENT_GROUP_PTR(), __jtest_temp_group_ptr); \
                                                                                                     \
