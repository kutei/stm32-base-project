    ...
    {                                                                                                                   \
        ...
        for (i = 0; i < CONTROLLER_MAX_COEFFS_LEN / 3; i++) {                                                           \
            ...
            JTEST_COUNT_CYCLES(for (j = 0; j < CONTROLLER_MAX_LEN; j++) {                                               \
                *((type*)controller_output_fut + j) = arm_pid_##suffix(&fut_pid_inst, controller_##suffix##_inputs[j]); \
            });                                                                                                         \
                                                                                                                        \
            for (j = 0; j < CONTROLLER_MAX_LEN; j++) {                                                                  \
                *((type*)controller_output_ref + j) = ref_pid_##suffix(&ref_pid_inst, controller_##suffix##_inputs[j]); \
            }                                                                                                           \
                                                                                                                        \
            /* Test correctness */                                                                                      \
