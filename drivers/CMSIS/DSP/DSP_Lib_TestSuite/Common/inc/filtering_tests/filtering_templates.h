 ...
 */
#define FILTERING_SNR_COMPARE_INTERFACE(block_size, output_type) FILTERING_SNR_COMPARE_INTERFACE_OFFSET(0, block_size, output_type)
...
#define FILTERING_SNR_COMPARE_INTERFACE_OFFSET(offset, block_size, output_type)                                                                    \
    do {                                                                                                                                           \
        TEST_CONVERT_AND_ASSERT_SNR(filtering_output_f32_ref, (output_type *)filtering_output_ref + offset, filtering_output_f32_fut,              \
                                    (output_type *)filtering_output_fut + offset, block_size, output_type, FILTERING_SNR_THRESHOLD_##output_type); \
    } while (0)
