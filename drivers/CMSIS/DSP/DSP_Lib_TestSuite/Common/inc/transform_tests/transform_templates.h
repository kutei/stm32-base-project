...
#define TRANSFORM_SNR_COMPARE_INTERFACE(block_size, output_type)                                                                              \
    do {                                                                                                                                      \
        TEST_CONVERT_AND_ASSERT_SNR(transform_fft_output_f32_ref, (output_type *)transform_fft_output_ref, transform_fft_output_f32_fut,      \
                                    (output_type *)transform_fft_output_fut, block_size, output_type, TRANSFORM_SNR_THRESHOLD_##output_type); \
    } while (0)
...
#define DCT_TRANSFORM_SNR_COMPARE_INTERFACE(block_size, output_type)                                                                               \
    do {                                                                                                                                           \
        TEST_CONVERT_AND_ASSERT_SNR(transform_fft_output_f32_ref, (output_type *)transform_fft_output_ref, transform_fft_output_f32_fut,           \
                                    (output_type *)transform_fft_output_fut, block_size, output_type, DCT4_TRANSFORM_SNR_THRESHOLD_##output_type); \
    } while (0)
...
#define TRANSFORM_PREPARE_INVERSE_INPUTS(input_ptr, fftlen, input_type, bytes)                                                                \
    do {                                                                                                                                      \
        ...
        for (i = 1; i < fftlen / 2; i++) {                                                                                                    \
            *((input_type *)transform_fft_input_fut + fftlen + 2 * i + 0) = *((input_type *)transform_fft_input_fut + fftlen - 2 * i + 0);    \
            *((input_type *)transform_fft_input_fut + fftlen + 2 * i + 1) = -(*((input_type *)transform_fft_input_fut + fftlen - 2 * i + 1)); \
        }                                                                                                                                     \
                                                                                                                                              \
